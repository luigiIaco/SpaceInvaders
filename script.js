...
function ship_move(mode) {
    switch (mode) {
        case "LEFT":
            if (SHIP_POS == 0) return true;
            ship_clear();
            SHIP_POS--;
            ship_add();
            break;
        case "RIGHT":
            if (SHIP_POS == SHIP_AREA.length - 1) return true;
            ship_clear();
            SHIP_POS++;
            ship_add();
            break;
...
function alien_move() {
    switch (ALIEN_DIR) {
        case "DX":
            for (let i = 0; i < ALIENS.length; i++) {
                if (
                    ALIENS[i].getAttribute("border") == "DX" &&
                    ALIENS[i].classList.contains("alien")
                ) {
                    return moveDown();
                }
            }
        ...
        case "SX":
            for (let i = 0; i < ALIENS.length; i++) {
                if (
                    ALIENS[i].getAttribute("border") == "SX" &&
                    ALIENS[i].classList.contains("alien")
                ) {
                    return moveDown();
                }
